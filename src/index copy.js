...
function loadFork(forkArray, apiUrl, page) {
    if (forkArray.length != 0) {
        ...
        getForkPerPagePromise(apiUrl, page + 1).then((forkArray, apiUrl, page) => {
            loadFork(forkArray, apiUrl, page);
        })
    }
}

async function getAllFork(apiUrl) {
    var page = 1;
    var stop = false;
    while (!stop) {
        await getForkPerPagePromise(apiUrl, page);
    }
}
